...
// 		return fmt.Errorf("can't read classifier file")
// 	}
// 	if err != nil {
// 		return 0, err
// 	}
...
// 	if len(rects) == 0 {
// 		return fmt.Errorf("no faces found on this picture")
// 	}
// 	main := mainFace(rects)
...
// 		area := (rect.Max.Y - rect.Min.Y) * (rect.Max.X - rect.Max.X)
// 		if area > maxArea {
// 			current = &rect
// 		}
// 	}
...
func Transform(mid Middler, file string, name string, outfile string) error {
	...
	if err != nil {
		return fmt.Errorf("can't load image %v", err)
	}
	middle, err := mid(img)
	if err != nil {
		return fmt.Errorf("can't load image %v", err)
